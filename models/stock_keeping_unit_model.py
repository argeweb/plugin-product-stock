...
class StockKeepingUnitModel(BasicModel):
    ...
    def sku_full_name(self):
        if self.sku_no is u'' or self.sku_no is None:
            return '%s-%s' % (self.sku_prev_name, self.name)
    ...
    category = Fields.CategoryProperty(verbose_name=u'所屬產品', kind=ProductModel)
